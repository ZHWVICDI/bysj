...
public class MediaServiceImpl implements MediaService {
    ...
    public void convert(String ffmpegPath,String videoPath, String fileName) {
        ...
        convert.add(VideoConstants.VIDEO_CONVERTED_PATH +newFileName);
        ProcessBuilder builder = new ProcessBuilder();
        try {
            builder.command(convert);
            // 如果此属性为 true，则任何由通过此对象的 start() 方法启动的后续子进程生成的错误输出都将与标准输出合并，
    ...
    public void cutPic(String ffmpegpath,String videoPath,String covername) {
        ...
        cutpic.add("800*450"); // 添加截取的图片大小为350*240
        cutpic.add("-f");
        cutpic.add("image2");
        cutpic.add(VideoConstants.PIC_CUTTED_PATH+covername); // 添加截取的图片的保存路径,包含文件名
